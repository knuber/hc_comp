SELECT 
	...
	PMNT "Remitance Type",
	CASE X.JRNLT WHEN 'PRIMAR' THEN CODE ELSE 'TOTALS' END "Withholding Code",
	SUBSTR(CASE x.jrnlt	WHEN 'PRIMAR' THEN r.cms_acct ELSE COALESCE(r.cms_tb, '00') || '0000101002' END, 7, 6) "GL Account",
	SUM(r.amount * CASE x.jrnlt WHEN 'PRIMAR'::text THEN '-1'::integer ELSE 1 END::numeric) FILTER (WHERE ADP_COMP = 'B3X') "B3X - Elyria",
	SUM(r.amount * CASE x.jrnlt WHEN 'PRIMAR'::text THEN '-1'::integer ELSE 1 END::numeric) FILTER (WHERE ADP_COMP = 'UDV') "UDV - Middlefield",
	SUM(r.amount * CASE x.jrnlt WHEN 'PRIMAR'::text THEN '-1'::integer ELSE 1 END::numeric) FILTER (WHERE ADP_COMP = 'U7H') "U7H - Sparks",
	SUM(r.amount * CASE x.jrnlt WHEN 'PRIMAR'::text THEN '-1'::integer ELSE 1 END::numeric) FILTER (WHERE ADP_COMP = 'U7J') "U7J - Sebring",
	SUM(r.amount * CASE x.jrnlt WHEN 'PRIMAR'::text THEN '-1'::integer ELSE 1 END::numeric) FILTER (WHERE ADP_COMP = 'U7C') "U7C - Non-Ohio Sales",
	SUM(r.amount * CASE x.jrnlt WHEN 'PRIMAR'::text THEN '-1'::integer ELSE 1 END::numeric) FILTER (WHERE ADP_COMP = 'U7E') "U7E - Grower Express"
	PAY_DATE,
	PMNT,
...
FROM 
	...
	CROSS JOIN (VALUES('PRIMAR'),('OFFSET')) x(jrnlt)
...
GROUP BY 
	...
	SUBSTR(
	    CASE x.jrnlt
		WHEN 'PRIMAR' THEN r.cms_acct
		ELSE COALESCE(r.cms_tb, '00') || '0000101002'
 END, 7, 6)
