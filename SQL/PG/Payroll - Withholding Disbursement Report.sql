SELECT 
	...
	SUM(r.amount *
	    CASE x.jrnlt
			WHEN 'PRIMAR'::text THEN '-1'::integer
			ELSE 1
	    END::numeric
	) FILTER (WHERE ADP_COMP = 'B3X') B3X,
	SUM(r.amount *
	    CASE x.jrnlt
			WHEN 'PRIMAR'::text THEN '-1'::integer
			ELSE 1
	    END::numeric
	) FILTER (WHERE ADP_COMP = 'UDV') UDV,
	SUM(r.amount *
	    CASE x.jrnlt
			WHEN 'PRIMAR'::text THEN '-1'::integer
			ELSE 1
	    END::numeric
	) FILTER (WHERE ADP_COMP = 'U7H') U7H,
	SUM(r.amount *
	    CASE x.jrnlt
			WHEN 'PRIMAR'::text THEN '-1'::integer
			ELSE 1
	    END::numeric
	) FILTER (WHERE ADP_COMP = 'U7J') U7J,
	SUM(r.amount *
	    CASE x.jrnlt
			WHEN 'PRIMAR'::text THEN '-1'::integer
			ELSE 1
	    END::numeric
	) FILTER (WHERE ADP_COMP = 'U7C') U7C,
	SUM(r.amount *
	    CASE x.jrnlt
			WHEN 'PRIMAR'::text THEN '-1'::integer
			ELSE 1
	    END::numeric
	) FILTER (WHERE ADP_COMP = 'U7E') U7E
FROM 
	payroll.adp_rp r
	INNER JOIN payroll.adp_code c on
		r.gl_descr::text = c.code::text AND 
		r.prim_offset::text = c.po::text
	CROSS JOIN unnest(ARRAY['PRIMAR'::text, 'OFFSET'::text]) x(jrnlt)
