...
WITH
	...
	NDAYS
		...
		SELECT 
			...
			N1CCYY FSYR, 
			KPMAXP PERDS, 
			FORMAT(N1FSPP,'0000') PERD, 
			FORMAT(N1FSYP,'0000') FSPR,  
			N1SD01 SDAT, 
			N1ED01 EDAT, 
			SUBSTRING(CAST(N1ED01 AS VARCHAR(MAX)),3,2)+SUBSTRING(CAST(N1ED01 AS VARCHAR(MAX)),6,2) CAPR,  
			DATEDIFF(day,N1SD01,N1ED01) --+ 1 NDAYS 
	...
	-------now build out initial orders list first (since it is a cross join and needs filtered before doing more joins)---
	 O (
		...
		SELECT
			...
			DHPOST, 
			------------------------periods------------------------------------------------------------------------
			FORMAT(DHARYR,'00')+FORMAT(DHARPR,'00') FSPR_INV,
			------------------------attributes---------------------------------------------------------------------
			...
			CASE F.FLAG
				WHEN 'REMAINDER' THEN 
					DDQTOI-DDQTSI
				WHEN 'SHIPMENT' THEN
					DIQTSH*CASE DHPOST WHEN 'Y' THEN 1 ELSE 0 END
			END FB_QTY,
			...
			CASE F.FLAG
				...
				WHEN 'SHIPMENT' THEN
					...
					CASE DDQTOI 
						WHEN 0 THEN 0 
						ELSE DDTOTI/DDQTOI 
					END*COALESCE(DIQTSH*CASE DHPOST WHEN 'Y' THEN 1 ELSE 0 END,DDQTSI)
			END FB_VAL_LOC
		FROM
			----------------------Order Data-----------------------------
			LGDAT.OCRI 
		...
		WHERE
			...
			(	
				...
				(F.FLAG = 'SHIPMENT' AND DIQTSH *CASE DHPOST WHEN 'Y' THEN 1 ELSE 0 END<>0)
			)
	)
SELECT
	COUNT(*)
