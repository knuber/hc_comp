SELECT
	...
	SUM(QTY*COALESCE(CGMATS, CHSTCS, Y0SMAT + Y0SOC + Y0SSHC)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_CUR_MAT_USD,
	SUM(QTY*COALESCE(CGLABS, Y0SLAB, 0)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_CUR_LAB_USD,
	SUM(QTY*COALESCE(CGBRVS, Y0SVBR, 0)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_CUR_VAR_USD,
	SUM(QTY*COALESCE(CGBRFS, Y0SFBR, 0)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_CUR_FIX_USD,
	SUM(QTY*COALESCE(CGSTOC, Y0SOTC, 0)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_CUR_OTH_USD,
	SUM(QTY*COALESCE(CNSTCS, COSTCS, Y3STCS, NPCOST)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_FUT_USD,
	SUM(QTY*COALESCE(CNMATS, COSTCS, Y3SMAT + Y3SOC + Y3SSHC)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_FUT_MAT_USD,
	SUM(QTY*COALESCE(CNLABS, Y3SLAB, 0)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_FUT_LAB_USD,
	SUM(QTY*COALESCE(CNBRVS, Y3SVBR, 0)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_FUT_VAR_USD,
	SUM(QTY*COALESCE(CNBRFS, Y3SFBR, 0)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_FUT_FIX_USD,
	SUM(QTY*COALESCE(CNSTOC, Y3SOTC, 0)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_FUT_OTH_USD,
	SUM(QTY*COALESCE(AVNWHT,AWNWHT)*CASE COALESCE(AVNWUN,AWNWUN) WHEN 'KG' THEN 2.2046 ELSE 1 END) POUNDS
FROM
	QGPL.FFBS0403
...
SELECT
	...
	FLAG,
	SUM(I_SHIPDATE) I_SHIPDATE,
	SUM(VALUE_LOCAL*CASE R_CURRENCY WHEN 'CA' THEN XR.RATE ELSE 1 END) REVENUE_USD,
	SUM(QTY*COALESCE(CGSTCS, CHSTCS, Y0STCS)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_CUR_USD,
	SUM(QTY*COALESCE(CGMATS, CHSTCS, Y0SMAT + Y0SOC + Y0SSHC)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_CUR_MAT_USD,
	SUM(QTY*COALESCE(CGLABS, Y0SLAB, 0)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_CUR_LAB_USD,
	SUM(QTY*COALESCE(CGBRVS, Y0SVBR, 0)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_CUR_VAR_USD,
	SUM(QTY*COALESCE(CGBRFS, Y0SFBR, 0)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_CUR_FIX_USD,
	SUM(QTY*COALESCE(CGSTOC, Y0SOTC, 0)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_CUR_OTH_USD,
	SUM(QTY*COALESCE(CNSTCS, COSTCS, Y3STCS)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_FUT_USD,
	SUM(QTY*COALESCE(CNMATS, COSTCS, Y3SMAT + Y3SOC + Y3SSHC)*CASE C_CURRENCY WHEN 'CA' THEN .75 ELSE 1 END) SCOGS_FUT_MAT_USD,
	SUM(QTY*COALESCE(CNLABS, Y3SLAB, 0)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_FUT_LAB_USD,
	SUM(QTY*COALESCE(CNBRVS, Y3SVBR, 0)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_FUT_VAR_USD,
	SUM(QTY*COALESCE(CNBRFS, Y3SFBR, 0)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_FUT_FIX_USD,
	SUM(QTY*COALESCE(CNSTOC, Y3SOTC, 0)*CASE C_CURRENCY WHEN 'CA' THEN XC.RATE ELSE 1 END) SCOGS_FUT_OTH_USD,
	SUM(QTY*COALESCE(AVNWHT,AWNWHT)*CASE COALESCE(AVNWUN,AWNWUN) WHEN 'KG' THEN 2.2046 ELSE 1 END) POUNDS
FROM
	QGPL.FFBSHIST
	LEFT OUTER JOIN LGDAT.ICSTP ON
